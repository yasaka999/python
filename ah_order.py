...
q_data1 = (
    "?qdi=queryc3ordersinglepoint&starttime="
    + yesterday
    + "&endtime="
    + yesterday
    + "&datetype=D&token="
    + token
    + "&sysid=t&userid=test1"
)
...
q_data2 = (
    "?qdi=queryc3ordermonthpoint&starttime="
    + yesterday
    + "&endtime="
    + yesterday
    + "&datetype=D&type=0%27%2C%273%27%2C%274%27%2C%275%27%2C%27&token="
    + token
    + "&sysid=t&userid=test1"
)
...
b_yesterday = datetime.date.today() + datetime.timedelta(-2)
...
total_sum_v = (
    p1_aaa * 5.0 + p2_aaa * 5.0 + (p3_aaa - p3_add) * 15 + p4_aaa * 9.9 + p5_aaa * 99
)
...
for i in range(nrows - 1, nrows):
    for j in range(ncols):
        ws0.cell(row=i + 1, column=j + 1).alignment = Alignment(
...
for a in range(nrows - 1, nrows):
    for b in (4, 7, 11, 14, 17):
        ...
        if ws0.cell(row=a + 1, column=b).value != 0:
            ws0.cell(row=a + 1, column=b).font = Font(
                size=10, bold=True, name="微软雅黑", color="FF0000"
            )
...
for i in range(nrows - 1, nrows):
    for j in range(ncols):
        ws1.cell(row=i + 1, column=j + 1).alignment = Alignment(
            horizontal="center", vertical="center"
        )
...
for a in range(nrows - 1, nrows):
    for b in (4, 7, 10, 13):
        ...
        if ws1.cell(row=a + 1, column=b).value != 0:
            ws1.cell(row=a + 1, column=b).font = Font(
                size=10, bold=True, name="微软雅黑", color="FF0000"
            )
...
# 处理总订购表
ws2 = wb.worksheets[2]
# 取最后一行值，分别加上当天的订购和退订数，目前一共5个产品
nrows = ws2.max_row
ncols = ws2.max_column
sum_all = [(yesterday)]
names = locals()
for i in range(1, 6):
    sum_all.append(ws2.cell(nrows, 2 * i).value + names["p" + str(i) + "_aaa"])
    sum_all.append(ws2.cell(nrows, 2 * i + 1).value + names["t" + str(i)])
...
for i in range(nrows, nrows + 1):
    for j in range(ncols):
        ws2.cell(row=i + 1, column=j + 1).alignment = Alignment(
            horizontal="center", vertical="center"
        )
        ws2.cell(row=i + 1, column=j + 1).border = border
        ws2.cell(row=i + 1, column=j + 1).font = font

ws2.cell(nrows + 1, 1).number_format = "m月d日"
...
# 第三方 SMTP 服务
# 发送邮件相关参数
smtpserver = "smtp.ahtv.tech:587"  # 发件服务器
sender = "ut01@ahtv.tech"  # 发件人邮箱
psw = "P&%&86894%@3Fe"  # 发件人密码
receiver = ["hans.han@utstarcom.cn"]  # 接收人
# 邮件标题
subjext = "订购明细数据" + str(yesterday)
# 获取附件信息
...
message.attach(body)
# 同一目录下的文件
att = MIMEText(open(new_filename, "rb").read(), "base64", "utf-8")
att["Content-Type"] = "application/octet-stream"
# filename附件名称
filename = "attachment; filename=" + new_filename
