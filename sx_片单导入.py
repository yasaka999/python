...
single_program_objects = [d for d in data if (d['Sequence']!='' and d['VolumnCount']!=1)]
...
#    print (series_objects[i])
    ...
    seriesdtl_object = [d for d in seriesdtl_objects if d['MAPPINGCODE'] == series_objects[i]['CODE']]
...
#    object_deal('Program', cases[i],code,mcode)
    tree = ET.ElementTree(root_xml)
    saveXML(root_xml, "../files/sx_single_program_%s.xml" %single_program_objects[i]['CODE'])
