...
for program_name in list1:
    ...
    sql_program = (
        ...
        "select programid,name,code,contentprovider,case status when '4' then '正常' when '9' then '已删除' else '其他' end,\
            case stockoutflag when '0' then '下线' when '1' then '上线' else '其他' end\
    ...
    sql_ctmshistoryeventdetail = (
        "select d.name,wm_concat(b.createdate) from \
            (select e.* from (select row_number() over(partition by c.domainid,c.objectaction order by c.underctmsnodeid ) rt,\
                c.underctmsnodeid, c.domainid, c.objectaction,to_char(c.dispatchdetailcreatedate,'yyyy-mm-dd hh24:mi:ss') createdate from ctmshistoryeventdetail c \
                    where c.domainid is not null and c.objectid= :1)e where e.rt=1)b,\
                        domain d  where b.domainid=d.domainid group by d.name")
    # 	print (sql)
    ...
    for i in program:
        if i[5] == "上线":
            ...
            for j in domainobjects:
                print (i[1], i[2], i[3], i[4], i[5], j[0], j[1], file=file6)
        else:
