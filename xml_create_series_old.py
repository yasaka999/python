...
def mapping_deal(type, dic):
    if type == 'Movie':
        ...
        mapping = ET.SubElement(mappings, "Mapping")
        mapping.set("Action", "REGIST")
        mapping.set("ElementCode", dic['SERIESCODE'])
        mapping.set("ElementID", dic['SERIESCODE'])
        mapping.set("ElementType", 'Series')
                                ...
                                A.OPIncharge,B.Status, A.Genre,A.overduetime,\
                                    A.NewPrice,A.Rating,A.ORGAIRDATE from series A, domainobjects B,\
                                         vsp C  where A.SeriesID=B.ObjID and A.VSPID=C.VSPID and B.ObjType='35' \
                                              and B.DomainID=1 and A.status='4' and rownum<150"
...
while rs:
    ...
    sqlpicture = "select a.code seriescode,c.code code, 'ftp://wacos:wacos@172.25.130.5//opt/wenke/expxml_program/data/picture/'||c.picture FileURL,\
        decode(b.picturetypeid,400,0,401,1,402,2,403,3,404,4,405,5,406,6,407,7,0) type,b.sequence sequence from series a,picturemap b,metapicture c \
              where b.objtype='5' and c.status='1'  and a.seriesid=b.objid and b.metapictureid=c.metapictureid and a.seriesid=%d" % (rs[0])
...
#   dealprogram(rs,rsmovie,rspicture)
    ...
    # create picture
    s_delcolumn = ('SERIESCODE', 'CODE', 'SEQUENCE', 'TYPE')
    for k in picture_object:
        object_deal('Picture', k, s_delcolumn)
