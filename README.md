...
### 1. 字典配置说明
- **系统配置页面**用于管理所有下拉选项字段（项目状态、阶段、问题严重等级、风险概率/影响/状态等）
- 修改字典后需点击「保存所有配置」按钮，系统使用批量保存 API 一次性提交所有变更
- **风险等级**(`risk_level`)是根据概率和影响自动计算的，包含：高(rl_h)、中(rl_m)、低(rl_l)
- 如果风险等级显示为英文代码而非中文，请检查数据库中是否存在 `risk_level` 分类的字典项

### 2. 首次部署必做
```bash
# 1. 初始化数据库表结构（首次启动会自动创建）
# 2. 执行基础数据初始化（必须！否则字典为空会导致界面显示异常）
cd backend && python init_data.py
```

### 3. 开发环境 vs 生产环境
- **开发**：前端 `npm run dev` + 后端 `uvicorn --reload`
- **生产**：前端 `npm run build` 生成 dist 目录，由 Nginx 托管；后端使用 `nohup` 或 systemd 守护进程运行

### 4. 已知限制
- 服务器 Node.js 版本较旧时，前端需在本地构建后再部署到服务器
- 数据库文件 (`pmo.db`) 建议定期备份，已配置每日凌晨 2 点自动备份
