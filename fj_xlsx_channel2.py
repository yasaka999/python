...
for sheet_name in xls.sheet_names:
    ...
    # 处理频道号的匹配
    def read_channel_mapping(file_path):
        channel_mapping = {}
    ...
    # 添加频道号列
    df_merged['频道号'] = df_merged['频道名称'].map(read_channel_mapping('../files/channelnum.csv'))

    # 按收视时长倒序排列
    df_merged = df_merged.sort_values(by='收视时长', ascending=False)

     # 选择需要输出的列
    selected_columns = ['频道名称','收视次数（PV）','收视人数（UV）', '收视时长','频道号']
