...
class EventHandler(ProcessEvent):
    def process_IN_CLOSE_WRITE(self, event):
        ...
        if src_name.endswith(".txt"):
            ...
            # 打开输入文件进行读取
            with open(src_dir + "/" + src_name, 'r') as input_file:
            ...
            # 打开输出文件进行写入
            with open(dst_dir + "/" + src_name, 'w') as output_file:
                # 将修正后的结果写入输出文件
                output_file.write(corrected_schedule)
            
            os.chmod(dst_dir + "/" + src_name, 0777)
            os.chmod(dst_dir + "/" + src_name + ".ok", 0777)
    ...
    for entry in schedules:
        ...
        for i in range(1, len(lines)-1):  # 从第2行开始，因为第一行不需要处理
            ...
            if int(current_end_time) > int(next_start_time):
                ...
                # 记录错误日志
                error_log.append("Fixed: " + current_line + " -> " + lines[i])
...
if __name__ == "__main__":
    ...
    error_log_file_name = "/opt/xhan/error.log"  # 错误日志文件名
